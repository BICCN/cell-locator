...
we need to manage a ``vtkMRMLMarkupsNode`` _and_ a ``vtkMRMLModelNode``. The
...
``vtkSlicerSplinesLogic::CreateModelFromContour`` `(see here)`__

__ https://github.com/BICCN/cell-locator/blob/master/Modules/Loadable/Splines/Logic/vtkSlicerSplinesLogic.cxx#L68-L142
...
__ https://github.com/BICCN/cell-locator/pull/169
...
to JSON by Slicer `(see here)`__. Any annotation-specific metadata (thickness, etc) is
included using ``Annotation.getMetdata``.

__ https://slicer.readthedocs.io/en/latest/developer_guide/modules/markups.html
